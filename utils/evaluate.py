...
def _update_topk(top_scores, top_idx, block_scores, block_start, K):
    ...
    b_scores, b_idx = torch.topk(block_scores, k=bk, dim=1)  
    b_idx = b_idx + block_start                              
    cand_scores = torch.cat([top_scores, b_scores], dim=1)   
    cand_idx    = torch.cat([top_idx,    b_idx],    dim=1)   
